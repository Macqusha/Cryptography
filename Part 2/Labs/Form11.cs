...
namespace Labs
{
    public partial class Form11 : Form
    {
        ...
        {
            InitializeComponent();
            f11_chooseHashFun.Text = "ГОСТ Р 34.11-2012 256 бит";
            f11_chooseHashFunRecieved.Text = "ГОСТ Р 34.11-2012 256 бит";
        ...
        private void f11_buttonNextPrimeQ_Click(object sender, EventArgs e)
        {
            f11_textQ.Text = new string(f11_textQ.Text.Where(t => char.IsDigit(t)).ToArray()); //очищаем Q от ненужных символов

            BigInteger ParamQ = 0;      //Считываем Q
            if (f11_textQ.TextLength > 0)
                ParamQ = BigInteger.Parse(f11_textQ.Text);

            BigInteger minQ = BigInteger.Pow(2, 255);
            BigInteger maxQ = BigInteger.Pow(2, 256) - 1;
            if (ParamQ < minQ || ParamQ > maxQ)
                ParamQ = 0;


            RandomNumberGenerator rng = RandomNumberGenerator.Create();
            if (ParamQ == 0) ParamQ = BigIntegerRandomUtils.RandomInRange(rng, minQ, maxQ); 
            if (++ParamQ % 2 == 0) ParamQ++;

            while (!RabinMiller.IsPrime(ParamQ, 3))
            {
                ParamQ += 2;
                if (ParamQ > maxQ)
                {
            ...
            f11_textQ.Text = Convert.ToString(ParamQ);
            f11_textP.Text = "";
        }
        private void f11_buttonNextPrimeP_Click(object sender, EventArgs e)
        {
            f11_textQ.Text = new string(f11_textQ.Text.Where(t => char.IsDigit(t)).ToArray());
            f11_textP.Text = new string(f11_textP.Text.Where(t => char.IsDigit(t)).ToArray());
            ...
            BigInteger minQ = BigInteger.Pow(2, 255);
            BigInteger maxQ = BigInteger.Pow(2, 256) - 1;
            if (ParamQ > minQ && ParamQ < maxQ && RabinMiller.IsPrime(ParamQ, 3))
            {
                BigInteger ParamP = 0;
                if (f11_textP.TextLength > 0)
                    ParamP = BigInteger.Parse(f11_textP.Text);

                BigInteger maxP = BigInteger.Pow(2,1024) - 1;
                BigInteger minP = BigInteger.Pow(2, 1023);
                if (ParamP < minP || ParamP > maxP)
                    ParamP = 0;
                RandomNumberGenerator rng = RandomNumberGenerator.Create();
                if (ParamP == 0) ParamP = BigIntegerRandomUtils.RandomInRange(rng, minP, maxP); ;
                if (++ParamP % 2 == 0) ParamP++;


                ParamP = ((ParamP / ParamQ) + 1) * ParamQ + 1;
                if (ParamP % 2 == 0) ParamP+= ParamQ;
                ...
                f11_textQ.Text = Convert.ToString(ParamQ);
                f11_textP.Text = Convert.ToString(ParamP);
        ...
        private void f11_buttonGetRandomA_Click(object sender, EventArgs e)
        {
            f11_textQ.Text = new string(f11_textQ.Text.Where(t => char.IsDigit(t)).ToArray());
            f11_textP.Text = new string(f11_textP.Text.Where(t => char.IsDigit(t)).ToArray());
            ...
            if (ParamQ > 0 && ParamP > 0 && ((ParamP - 1) % ParamQ == 0) && RabinMiller.IsPrime(ParamQ, 3) && RabinMiller.IsPrime(ParamP, 3))
            {
                RandomNumberGenerator rng = RandomNumberGenerator.Create();
                BigInteger ParamA = 0;

                while (NumberTheory.BinaryModPow(ParamA, ParamQ, ParamP) != 1)
                {
                    BigInteger d = BigIntegerRandomUtils.RandomInRange(rng, 2, ParamQ - 1);
                    BigInteger b = (ParamP - 1) / ParamQ;
                    BigInteger f = NumberTheory.BinaryModPow(d, b, ParamP);
                    if (f != 1)
                    {
                        ParamA = f;
                    }
                }
                f11_textA.Text = Convert.ToString(ParamA);
            }
            else
            {
                MessageBox.Show("Сначала подберите допустимые P и Q.");
            }

            
        }
        private void f11_buttonSetParams_Click(object sender, EventArgs e)
        {
            f11_textQ.Text = new string(f11_textQ.Text.Where(t => char.IsDigit(t)).ToArray());
            f11_textP.Text = new string(f11_textP.Text.Where(t => char.IsDigit(t)).ToArray());
            f11_textA.Text = new string(f11_textA.Text.Where(t => char.IsDigit(t)).ToArray());
            ...
            BigInteger ParamA = 0;
            if (f11_textA.TextLength > 0)
                ParamA = BigInteger.Parse(f11_textA.Text);

            if (ParamP > 0 && ParamQ > 0 && ParamA > 0 && ((ParamP -1) % ParamQ == 0) && NumberTheory.BinaryModPow(ParamA, ParamQ, ParamP) == 1
                && RabinMiller.IsPrime(ParamQ, 3) && RabinMiller.IsPrime(ParamP, 3))

            {
                f11_textP.ReadOnly = true;
                f11_textQ.ReadOnly = true;
                f11_textA.ReadOnly = true;
                f11_buttonNextPrimeQ.Enabled = false;
                f11_buttonNextPrimeP.Enabled = false;
                f11_buttonGetRandomA.Enabled = false;
                f11_chooseHashFun.Enabled = false;
                f11_buttonSetParams.Enabled = false;
                f11_buttonClearParams.Enabled = true;

                f11_textSessionKey.ReadOnly = false;
                f11_textSecretKey.ReadOnly = false;
                f11_button_GetRandomSessionK.Enabled = true;
                f11_button_GetRandomSecret.Enabled = true;
                f11_button_SetSecretKey.Enabled = true;
                f11_button_ClearSecretKey.Enabled = false;//в случае сброса тоже
                f11_buttonSign.Enabled = false;//в случае сброса тоже
            }
        }
        private void f11_buttonClearParams_Click(object sender, EventArgs e)
        {
            f11_textP.ReadOnly = false;
            f11_textQ.ReadOnly = false;
            f11_textA.ReadOnly = false;
            f11_buttonNextPrimeQ.Enabled = true;
            f11_buttonNextPrimeP.Enabled = true;
            f11_buttonGetRandomA.Enabled = true;
            f11_chooseHashFun.Enabled = true;
            f11_buttonSetParams.Enabled = true;
            f11_buttonClearParams.Enabled = false;

            f11_textSessionKey.ReadOnly = true;
            f11_textSecretKey.ReadOnly = true;
            f11_button_GetRandomSessionK.Enabled = false;
            f11_button_GetRandomSecret.Enabled = false;
            f11_button_SetSecretKey.Enabled = false;
            f11_button_ClearSecretKey.Enabled = false;//аналогично установке
            f11_buttonSign.Enabled = false;//аналогично установке

            f11_textSessionKey.Text = "";
            f11_textSecretKey.Text = "";
            f11_textOpenKey.Text = "";
            f11_textBoxSign.Text = "";
            f11_textBoxSignR.Text = "";

        }

        private void f11_button_GetRandomSecret_Click(object sender, EventArgs e)
        {
            RandomNumberGenerator rng = RandomNumberGenerator.Create();
            BigInteger Q = BigInteger.Parse(f11_textQ.Text);
            BigInteger A = BigInteger.Parse(f11_textA.Text);
            BigInteger P = BigInteger.Parse(f11_textP.Text);
            BigInteger x = BigIntegerRandomUtils.RandomInRange(rng, 2, Q - 1);
            BigInteger y = NumberTheory.BinaryModPow(A, x, P);

            f11_textSecretKey.Text = Convert.ToString(x);
            f11_textOpenKey.Text = Convert.ToString(y);
        }

        private void f11_button_GetRandomSessionK_Click(object sender, EventArgs e)
        {
            RandomNumberGenerator rng = RandomNumberGenerator.Create();
            BigInteger Q = BigInteger.Parse(f11_textQ.Text);
            BigInteger A = BigInteger.Parse(f11_textA.Text);
            BigInteger P = BigInteger.Parse(f11_textP.Text);
            BigInteger k = BigIntegerRandomUtils.RandomInRange(rng, 2, Q - 1);

            f11_textSessionKey.Text = Convert.ToString(k);
        }

        private void f11_button_SetSecretKey_Click(object sender, EventArgs e)
        {
            BigInteger Q = BigInteger.Parse(f11_textQ.Text);
            BigInteger A = BigInteger.Parse(f11_textA.Text);
            BigInteger P = BigInteger.Parse(f11_textP.Text);

            f11_textSecretKey.Text = new string(f11_textSecretKey.Text.Where(t => char.IsDigit(t)).ToArray()); 
            f11_textSessionKey.Text = new string(f11_textSessionKey.Text.Where(t => char.IsDigit(t)).ToArray()); 

            BigInteger secret = 0;     
            if (f11_textSecretKey.TextLength > 0)
            {
                secret = BigInteger.Parse(f11_textSecretKey.Text) % P;
                f11_textSecretKey.Text = Convert.ToString(secret);
            }                
            BigInteger session = 0;
            if (f11_textSessionKey.TextLength > 0)
            {
                session = BigInteger.Parse(f11_textSessionKey.Text) % P;
                f11_textSessionKey.Text = Convert.ToString(session);
            }

            BigInteger y = NumberTheory.BinaryModPow(A, secret, P);
            f11_textOpenKey.Text = Convert.ToString(y);

            if (1 < session && session < Q && 1 < secret && secret < Q)
            {
                f11_textSessionKey.ReadOnly = true;
                f11_textSecretKey.ReadOnly = true;
                f11_button_GetRandomSessionK.Enabled = false;
                f11_button_GetRandomSecret.Enabled = false;
                f11_button_SetSecretKey.Enabled = false;
                f11_button_ClearSecretKey.Enabled = true;
                f11_buttonSign.Enabled = true;
            }
        }

        private void f11_button_ClearSecretKey_Click(object sender, EventArgs e)
        {
            f11_textSessionKey.ReadOnly = false;
            f11_textSecretKey.ReadOnly = false;
            f11_button_GetRandomSessionK.Enabled = true;
            f11_button_GetRandomSecret.Enabled = true;
            f11_button_SetSecretKey.Enabled = true;
            f11_button_ClearSecretKey.Enabled = false;
            f11_buttonSign.Enabled = false;
        }

        private void f11_buttonSign_Click(object sender, EventArgs e)
        {
            BigInteger P = BigInteger.Parse(f11_textP.Text);
            BigInteger Q = BigInteger.Parse(f11_textQ.Text);
            BigInteger A = BigInteger.Parse(f11_textA.Text);
            BigInteger X = BigInteger.Parse(f11_textSecretKey.Text);
            BigInteger K = BigInteger.Parse(f11_textSessionKey.Text);
            ...
            if (Load.ShowDialog() == DialogResult.OK)
            {
                ...
                if (f11_chooseHashFun.Text == "MD5")
                    fileHash = HashFunctions.MD5.GetHashMD5(file);
                if (f11_chooseHashFun.Text == "SHA-1")
                    fileHash = HashFunctions.SHA1.GetHashSHA1(file);
                if (f11_chooseHashFun.Text == "ГОСТ Р 34.11-2012 256 бит")
                    fileHash = HashFunctions.GOST.GetHashGOST(file, 256);
                if (f11_chooseHashFun.Text == "ГОСТ Р 34.11-2012 512 бит")
                    fileHash = HashFunctions.GOST.GetHashGOST(file, 512);
            }

            BigInteger hashDigit = new BigInteger(Encoding.Default.GetBytes(fileHash));
            
            BigInteger R = NumberTheory.BinaryModPow(A, K, P) % Q;
            BigInteger S = (K * hashDigit + X * R) % Q; 

            if (S > 0 && R > 0)
            {
                f11_textBoxSign.Text = Convert.ToString(S);
                f11_textBoxSignR.Text = Convert.ToString(R);

                //f11_button_ClearSecretKey.PerformClick();
                //f11_textSessionKey.Focus();
            }
            else
            {
                f11_textBoxSign.Text = "";
                f11_textBoxSignR.Text = "";
                MessageBox.Show("Пожалуйста, выберите другой сессионный ключ", "Ошибка");
        ...
        private void f11_buttonSendSign_Click(object sender, EventArgs e)
        {
            f11_chooseHashFunRecieved.Text = f11_chooseHashFun.Text;
            f11_textP_Recieved.Text = f11_textP.Text;
            f11_textQ_Recieved.Text = f11_textQ.Text;
            f11_textA_Recieved.Text = f11_textA.Text;
            f11_textOpenKeyRecieved.Text = f11_textOpenKey.Text;
            f11_textBoxSignS_Recieved.Text = f11_textBoxSign.Text;
            f11_textBoxSignR_Recieved.Text = f11_textBoxSignR.Text;
        }

        private void f11_buttonCheckSign_Click(object sender, EventArgs e)
        {
            f11_textP_Recieved.Text = new string(f11_textP_Recieved.Text.Where(t => char.IsDigit(t)).ToArray()); 
            f11_textQ_Recieved.Text = new string(f11_textQ_Recieved.Text.Where(t => char.IsDigit(t)).ToArray()); 
            f11_textA_Recieved.Text = new string(f11_textA_Recieved.Text.Where(t => char.IsDigit(t)).ToArray()); 
            f11_textOpenKeyRecieved.Text = new string(f11_textOpenKeyRecieved.Text.Where(t => char.IsDigit(t)).ToArray()); 
            f11_textBoxSignR_Recieved.Text = new string(f11_textBoxSignR_Recieved.Text.Where(t => char.IsDigit(t)).ToArray()); 
            f11_textBoxSignS_Recieved.Text = new string(f11_textBoxSignS_Recieved.Text.Where(t => char.IsDigit(t)).ToArray()); 

            if (f11_textP_Recieved.TextLength * f11_textQ_Recieved.TextLength * f11_textA_Recieved.TextLength
                * f11_textOpenKeyRecieved.TextLength * f11_textBoxSignR_Recieved.TextLength * f11_textBoxSignS_Recieved.TextLength != 0)
            {
                //если хоть одно поле пустое, подпись не проверится
                BigInteger P = BigInteger.Parse(f11_textP_Recieved.Text);
                BigInteger Q = BigInteger.Parse(f11_textQ_Recieved.Text);
                BigInteger A = BigInteger.Parse(f11_textA_Recieved.Text);
                BigInteger Y = BigInteger.Parse(f11_textOpenKeyRecieved.Text);
                BigInteger R = BigInteger.Parse(f11_textBoxSignR_Recieved.Text);
                BigInteger S = BigInteger.Parse(f11_textBoxSignS_Recieved.Text);
                ...
                BigInteger antiHash = NumberTheory.Foo(hashDigit, Q);
                BigInteger u1 = (S * antiHash) % Q;
                BigInteger u2 = (((-R * antiHash) % Q) + Q) % Q;
                BigInteger v =
                   ((NumberTheory.BinaryModPow(A, u1, P) * NumberTheory.BinaryModPow(Y, u2, P)) % P) % Q; 

                if (0 < S && S < Q && 0 < R && R < Q && v == R)
                {
                    f11_textVerificationResult.Text = "Подпись верна";
                }
                else
                {
                    f11_textVerificationResult.Text = "Подпись неверна";
                }
