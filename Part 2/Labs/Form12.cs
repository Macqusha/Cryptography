...
namespace Labs
{
    public partial class Form12 : Form
    {
        ...
        {
            InitializeComponent();
            f12_chooseHashFun.Text = "SHA-1";
            f12_chooseHashFunRecieved.Text = "SHA-1";
        }
        ...
        private void f12_buttonNextPrimeQ_Click(object sender, EventArgs e)
        {
            f12_textQ.Text = new string(f12_textQ.Text.Where(t => char.IsDigit(t)).ToArray()); //очищаем Q от ненужных символов

            BigInteger ParamQ = 0;      //Считываем Q
            if (f12_textQ.TextLength > 0)
                ParamQ = BigInteger.Parse(f12_textQ.Text);

            BigInteger minQ = BigInteger.Pow(2, 159);
            BigInteger maxQ = BigInteger.Pow(2, 160) - 1;
            ...
            f12_textQ.Text = Convert.ToString(ParamQ);
            f12_textP.Text = "";
        }
        private void f12_buttonNextPrimeP_Click(object sender, EventArgs e)
        {
            f12_textQ.Text = new string(f12_textQ.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textP.Text = new string(f12_textP.Text.Where(t => char.IsDigit(t)).ToArray());
            ...
            {
                BigInteger ParamP = 0;
                if (f12_textP.TextLength > 0)
                    ParamP = BigInteger.Parse(f12_textP.Text);
                ...
                f12_textQ.Text = Convert.ToString(ParamQ);
                f12_textP.Text = Convert.ToString(ParamP);
            }
            else
            {
                f12_textQ.Text = "";
        ...
        private void f12_buttonGetRandomA_Click(object sender, EventArgs e)
        {
            f12_textQ.Text = new string(f12_textQ.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textP.Text = new string(f12_textP.Text.Where(t => char.IsDigit(t)).ToArray());
            ...
            if (ParamQ > 0 && ParamP > 0 && ((ParamP - 1) % ParamQ == 0) && RabinMiller.IsPrime(ParamQ, 3) && RabinMiller.IsPrime(ParamP, 3))
            {
                ...
                f12_textA.Text = Convert.ToString(ParamA);
            }
            else
            {
        ...
        private void f12_buttonSetParams_Click(object sender, EventArgs e)
        {
            f12_textQ.Text = new string(f12_textQ.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textP.Text = new string(f12_textP.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textA.Text = new string(f12_textA.Text.Where(t => char.IsDigit(t)).ToArray());
            ...
            BigInteger ParamA = 0;
            if (f12_textA.TextLength > 0)
                ParamA = BigInteger.Parse(f12_textA.Text);
            ...
            {
                f12_textP.ReadOnly = true;
                f12_textQ.ReadOnly = true;
                f12_textA.ReadOnly = true;
                f12_buttonNextPrimeQ.Enabled = false;
                f12_buttonNextPrimeP.Enabled = false;
                f12_buttonGetRandomA.Enabled = false;
                f12_chooseHashFun.Enabled = false;
                f12_buttonSetParams.Enabled = false;
                f12_buttonClearParams.Enabled = true;

                f12_textSessionKey.ReadOnly = false;
                f12_textSecretKey.ReadOnly = false;
                f12_button_GetRandomSessionK.Enabled = true;
                f12_button_GetRandomSecret.Enabled = true;
                f12_button_SetSecretKey.Enabled = true;
                f12_button_ClearSecretKey.Enabled = false;//в случае сброса тоже
                f12_buttonSign.Enabled = false;//в случае сброса тоже
            }
        }
        private void f12_buttonClearParams_Click(object sender, EventArgs e)
        {
            f12_textP.ReadOnly = false;
            f12_textQ.ReadOnly = false;
            f12_textA.ReadOnly = false;
            f12_buttonNextPrimeQ.Enabled = true;
            f12_buttonNextPrimeP.Enabled = true;
            f12_buttonGetRandomA.Enabled = true;
            f12_chooseHashFun.Enabled = true;
            f12_buttonSetParams.Enabled = true;
            f12_buttonClearParams.Enabled = false;

            f12_textSessionKey.ReadOnly = true;
            f12_textSecretKey.ReadOnly = true;
            f12_button_GetRandomSessionK.Enabled = false;
            f12_button_GetRandomSecret.Enabled = false;
            f12_button_SetSecretKey.Enabled = false;
            f12_button_ClearSecretKey.Enabled = false;//аналогично установке
            f12_buttonSign.Enabled = false;//аналогично установке

            f12_textSessionKey.Text = "";
            f12_textSecretKey.Text = "";
            f12_textOpenKey.Text = "";
            f12_textBoxSign.Text = "";
            f12_textBoxSignR.Text = "";

        }

        private void f12_button_GetRandomSecret_Click(object sender, EventArgs e)
        {
            RandomNumberGenerator rng = RandomNumberGenerator.Create();
            BigInteger Q = BigInteger.Parse(f12_textQ.Text);
            BigInteger A = BigInteger.Parse(f12_textA.Text);
            BigInteger P = BigInteger.Parse(f12_textP.Text);
            BigInteger x = BigIntegerRandomUtils.RandomInRange(rng, 2, Q - 1);
            BigInteger y = NumberTheory.BinaryModPow(A, x, P);

            f12_textSecretKey.Text = Convert.ToString(x);
            f12_textOpenKey.Text = Convert.ToString(y);
        }

        private void f12_button_GetRandomSessionK_Click(object sender, EventArgs e)
        {
            RandomNumberGenerator rng = RandomNumberGenerator.Create();
            BigInteger Q = BigInteger.Parse(f12_textQ.Text);
            BigInteger A = BigInteger.Parse(f12_textA.Text);
            ...
            f12_textSessionKey.Text = Convert.ToString(k);
        }

        private void f12_button_SetSecretKey_Click(object sender, EventArgs e)
        {
            BigInteger Q = BigInteger.Parse(f12_textQ.Text);
            BigInteger A = BigInteger.Parse(f12_textA.Text);
            BigInteger P = BigInteger.Parse(f12_textP.Text);

            f12_textSecretKey.Text = new string(f12_textSecretKey.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textSessionKey.Text = new string(f12_textSessionKey.Text.Where(t => char.IsDigit(t)).ToArray());

            BigInteger secret = 0;
            if (f12_textSecretKey.TextLength > 0)
            {
                secret = BigInteger.Parse(f12_textSecretKey.Text) % P;
                f12_textSecretKey.Text = Convert.ToString(secret);
            }
            BigInteger session = 0;
            if (f12_textSessionKey.TextLength > 0)
            {
                session = BigInteger.Parse(f12_textSessionKey.Text) % P;
                f12_textSessionKey.Text = Convert.ToString(session);
            }
            ...
            if (1 < session && session < Q && 1 < secret && secret < Q)
            {
                ...
                f12_button_GetRandomSecret.Enabled = false;
                f12_button_SetSecretKey.Enabled = false;
                f12_button_ClearSecretKey.Enabled = true;
                f12_buttonSign.Enabled = true;
            }
        }

        private void f12_button_ClearSecretKey_Click(object sender, EventArgs e)
        {
            f12_textSessionKey.ReadOnly = false;
            f12_textSecretKey.ReadOnly = false;
            f12_button_GetRandomSessionK.Enabled = true;
            f12_button_GetRandomSecret.Enabled = true;
            f12_button_SetSecretKey.Enabled = true;
            f12_button_ClearSecretKey.Enabled = false;
            f12_buttonSign.Enabled = false;
        }

        private void f12_buttonSign_Click(object sender, EventArgs e)
        {
            BigInteger P = BigInteger.Parse(f12_textP.Text);
            BigInteger Q = BigInteger.Parse(f12_textQ.Text);
            BigInteger A = BigInteger.Parse(f12_textA.Text);
            BigInteger X = BigInteger.Parse(f12_textSecretKey.Text);
            BigInteger K = BigInteger.Parse(f12_textSessionKey.Text);
            ...
            if (Load.ShowDialog() == DialogResult.OK)
            {
                ...
                if (f12_chooseHashFun.Text == "MD5")
                    fileHash = HashFunctions.MD5.GetHashMD5(file);
                if (f12_chooseHashFun.Text == "SHA-1")
                    fileHash = HashFunctions.SHA1.GetHashSHA1(file);
                if (f12_chooseHashFun.Text == "ГОСТ Р 34.11-2012 256 бит")
                    fileHash = HashFunctions.GOST.GetHashGOST(file, 256);
                if (f12_chooseHashFun.Text == "ГОСТ Р 34.11-2012 512 бит")
            ...
            BigInteger antiK = NumberTheory.Foo(K, Q);
            BigInteger S = (antiK *(hashDigit + X*R)) % Q;

            if (S > 0 && R > 0)
            {
                f12_textBoxSign.Text = Convert.ToString(S);
                f12_textBoxSignR.Text = Convert.ToString(R);

                //f12_button_ClearSecretKey.PerformClick();
                //f12_textSessionKey.Focus();
            }
            else
            {
                f12_textBoxSign.Text = "";
        ...
        private void f12_buttonSendSign_Click(object sender, EventArgs e)
        {
            f12_chooseHashFunRecieved.Text = f12_chooseHashFun.Text;
            f12_textP_Recieved.Text = f12_textP.Text;
            f12_textQ_Recieved.Text = f12_textQ.Text;
            f12_textA_Recieved.Text = f12_textA.Text;
            f12_textOpenKeyRecieved.Text = f12_textOpenKey.Text;
            f12_textBoxSignS_Recieved.Text = f12_textBoxSign.Text;
            f12_textBoxSignR_Recieved.Text = f12_textBoxSignR.Text;
        }

        private void f12_buttonCheckSign_Click(object sender, EventArgs e)
        {
            f12_textP_Recieved.Text = new string(f12_textP_Recieved.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textQ_Recieved.Text = new string(f12_textQ_Recieved.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textA_Recieved.Text = new string(f12_textA_Recieved.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textOpenKeyRecieved.Text = new string(f12_textOpenKeyRecieved.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textBoxSignR_Recieved.Text = new string(f12_textBoxSignR_Recieved.Text.Where(t => char.IsDigit(t)).ToArray());
            f12_textBoxSignS_Recieved.Text = new string(f12_textBoxSignS_Recieved.Text.Where(t => char.IsDigit(t)).ToArray());

            if (f12_textP_Recieved.TextLength * f12_textQ_Recieved.TextLength * f12_textA_Recieved.TextLength
                * f12_textOpenKeyRecieved.TextLength * f12_textBoxSignR_Recieved.TextLength * f12_textBoxSignS_Recieved.TextLength != 0)
            {
                //если хоть одно поле пустое, подпись не проверится
                BigInteger P = BigInteger.Parse(f12_textP_Recieved.Text);
                BigInteger Q = BigInteger.Parse(f12_textQ_Recieved.Text);
                BigInteger A = BigInteger.Parse(f12_textA_Recieved.Text);
                BigInteger Y = BigInteger.Parse(f12_textOpenKeyRecieved.Text);
                BigInteger R = BigInteger.Parse(f12_textBoxSignR_Recieved.Text);
                BigInteger S = BigInteger.Parse(f12_textBoxSignS_Recieved.Text);
                ...
                BigInteger antiS = NumberTheory.Foo(S, Q);
                BigInteger u1 = (hashDigit * antiS) % Q;
                BigInteger u2 = (R * antiS) % Q;
                BigInteger v =
                   ((NumberTheory.BinaryModPow(A, u1, P) * NumberTheory.BinaryModPow(Y, u2, P)) % P) % Q;

                if (0 < S && S < Q && 0 < R && R < Q && v == R)
                {
                    f12_textVerificationResult.Text = "Подпись верна";
                }
                else
                {
                    f12_textVerificationResult.Text = "Подпись неверна";
                }
